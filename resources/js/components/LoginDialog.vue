<template>
	<div>
		<Button @click="visible = true" class="login-button" buttonColor="red" buttonStyle="filled"
			buttonSize="normal">LOG IN</Button>

		<Dialog v-model:visible="visible" modal header="LOG IN"
			:style="{ width: '400px', backgroundColor: '#212121', color: 'white', borderColor: '#3B3B3B' }"
			class="login-dialog" @update:visible="onDialogClose">
			<div class="login-container">
				<form @submit.prevent="submitLogin">
					<input v-model="loginForm.email" type="email" class="form-control mb-2" placeholder="Email"
						required />
					...
					<div class="password-container">
						<input :type="passwordFieldType" v-model="loginForm.password" class="form-control mb-2"
					...
					<div class="form-check">
						<input class="form-check-input" type="checkbox" v-model="loginForm.remember"
				...
				<p class="text-center mt-3 d-flex justify-content-center">
					You don’t have an account? <a href="#" class="create-account ms-2"
						@click.prevent="openRegisterDialog">Create one</a>
				</p>
			</div>
		</Dialog>
	</div>
